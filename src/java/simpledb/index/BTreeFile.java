...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
        ...
        if (pid.pgcateg() == BTreePageId.ROOT_PTR) {
            BTreeRootPtrPage rootPtrPage = (BTreeRootPtrPage) getPage(tid, dirtypages, pid, Permissions.READ_ONLY);
            pid = rootPtrPage.getRootId();
        }
        ...
        assert pid.pgcateg() == BTreePageId.LEAF : "this page must be a leaf page";
	...
	public BTreeLeafPage findLeafPageReverse(TransactionId tid,
                                              BTreePageId pid,
                                              Field f)
					throws DbException, TransactionAbortedException {
        ...
        while (pid.pgcateg() == BTreePageId.INTERNAL) {
            ...
            while (it.hasNext()) {
                entry = it.next();
                if (f == null || f.compare(Op.GREATER_THAN_OR_EQ, entry.getKey())) {
                    pid = entry.getRightChild();
                    break;
                }
            }
	...
	public BTreeLeafPage splitLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
        ...
        Field newKey = newPage.getTuple(0).getField(keyField);
        BTreeEntry newEntry = new BTreeEntry(newKey, page.getId(), newPage.getId());
        ...
        if (field.compare(Op.LESS_THAN, newKey)) {
            return page;
        }
        return newPage;
	}
	...
	public void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
        ...
        int stealNum = (numTuples + siblingNumTuples)/2 - numTuples;
        assert stealNum > 0;
	...
	public void stealFromLeftInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
        ...
        int leftNumEntries = leftSibling.getNumEntries();
        int rightNumEntries = page.getNumEntries();
        int stealNum = (leftNumEntries + rightNumEntries)/2 - rightNumEntries;
        assert stealNum > 0;
	...
	public void stealFromRightInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		// the corresponding parent entry. Be sure to update the parent
		// pointers of all children in the entries that were moved.
        int leftNumEntries = page.getNumEntries();
        int rightNumEntries = rightSibling.getNumEntries();
        int stealNum = (leftNumEntries+rightNumEntries)/2 - leftNumEntries;
        assert stealNum > 0;
...
class BTreeFileReverseIterator extends AbstractDbFileIterator {
	...
	public void open() throws DbException, TransactionAbortedException {
		BTreeRootPtrPage rootPtr = (BTreeRootPtrPage) Database.getBufferPool().getPage(
				tid, BTreeRootPtrPage.getId(f.getId()), Permissions.READ_ONLY);
		BTreePageId root = rootPtr.getRootId();
		curp = f.findLeafPageReverse(tid, root, null);
...
class BTreeSearchReverseIterator extends AbstractDbFileIterator {
	...
	public void open() throws DbException, TransactionAbortedException {
		...
		if(ipred.getOp() == Op.EQUALS || ipred.getOp() == Op.LESS_THAN 
			...
			curp = f.findLeafPageReverse(tid, root, ipred.getField());
		}
		else {
			curp = f.findLeafPageReverse(tid, root, null);
		}
